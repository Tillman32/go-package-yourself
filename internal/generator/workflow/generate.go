...
func newWorkflow(projectName string, projectRepo string, goConfig model.Go, release model.Release, dockerCfg model.Docker, npmCfg model.NPM, homebrewCfg model.Homebrew, chocolateyCfg model.Chocolatey, matrixEntries []MatrixEntry, tagPatterns []string) WorkflowDoc {
	...
	if npmCfg.Enabled {
		jobs["publish-npm"] = publishNpmJob(projectName, npmCfg)
	}

	if homebrewCfg.Enabled {
		jobs["publish-homebrew"] = publishHomebrewJob(projectName, homebrewCfg)
	}

	if chocolateyCfg.Enabled {
		jobs["publish-chocolatey"] = publishChocolateyJob(projectName, chocolateyCfg)
	}
...
func publishNpmJob(projectName string, npmCfg model.NPM) WorkflowJob {
	packageName := npmCfg.PackageName
	if packageName == "" {
...
func publishHomebrewJob(projectName string, homebrewCfg model.Homebrew) WorkflowJob {
	formulaName := homebrewCfg.FormulaName
	if formulaName == "" {
		formulaName = capitalizeWords(projectName)
	}
	formulaPath := fmt.Sprintf("packaging/homebrew/%s.rb", formulaName)
...
# Copy committed generated formula to the fork
cd homebrew-core
FORMULA_FILE="../%s"
...
gh pr create \
  ...
  --body "Generated by [gpy](https://github.com/users/gpy-bot/repos)" \
	  --head "$(git config --global user.name):$BRANCH_NAME"`, formulaPath),
			},
		},
	}
}
...
func publishChocolateyJob(projectName string, chocolateyCfg model.Chocolatey) WorkflowJob {
	packageID := chocolateyCfg.PackageID
	if packageID == "" {
	...
	return WorkflowJob{
		...
		Steps: []WorkflowStep{
			...
			},
			{
				...
				Shell: "pwsh",
				Run: fmt.Sprintf(`# Navigate to committed generated package and pack it
cd "packaging\chocolatey\%s"
choco pack`,
					packageID,
				),
			},
			{
				...
				Run: fmt.Sprintf(`# Push the generated .nupkg to Chocolatey.org
cd "packaging\chocolatey\%s"
