...
2. **`gpy package`**: Reads config → generates npm/homebrew/chocolatey/docker artifacts
3. **`gpy workflow --write`**: Regenerate or update the GitHub Actions workflow separately
4. **`gpy package --sync`** / **`gpy workflow --sync`**: Re-run after updating `gpy.yaml` to keep artifacts in sync
...
- `--sync`: Regenerate and overwrite existing artifacts after updating `gpy.yaml`
...
- `--sync`: Regenerate and overwrite existing workflow after updating `gpy.yaml`
